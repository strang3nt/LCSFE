...
impl MuAld {
    pub fn new(
        ...
        let formula =
            mu_calc_parser::mu_calc_parser(&lts.labels).parse(formula).unwrap();
...
impl SpecOutput for MuAld {
    fn pre_proc(
        ...
        let (fix_system, moves) =
            mu_calc_parser::mucalc_to_fix_system(&self.formula, &self.lts)?;
    ...
    ) -> Result<sem_sfe_common::VerificationOutput, Box<dyn std::error::Error>>
    {
        ...
        let start = Instant::now();
        let result = local_algorithm.local_check(
            sem_sfe_algorithm::algorithm::Position::Eve(EvePos {
        ...
        let result =
            format!("The property {} from state {}", result, self.state);
